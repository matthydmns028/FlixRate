...
const Friends = (() => {
  ...
  // Cloud Accessors
  function getFriends() { return window.cloudProfile?.friends || []; }
  function getSentReqs() { return window.cloudProfile?.sentReqs || []; }
  function getIncoming() { return window.cloudProfile?.incomingReqs || []; }
  ...
  async function findUsers(queryStr) {
    try {
      ...
      let users = snap.docs.map(d => d.data().username).filter(u => u && u !== myUsername);
      if (queryStr) {
  ...
  async function cancelRequest(toUsername) {
    window.cloudProfile.sentReqs = getSentReqs().filter((u) => u !== toUsername);
    ...
    try {
      ...
      if (!snap.empty) {
        const otherId = snap.docs[0].id;
        const otherData = snap.docs[0].data();
        const incoming = (otherData.incomingReqs || []).filter(u => u !== window.cloudProfile.username);
        await updateDoc(doc(db, "users", otherId), { incomingReqs: incoming });
      }
  ...
  async function acceptRequest(fromUsername) {
    window.cloudProfile.incomingReqs = getIncoming().filter((u) => u !== fromUsername);
    if (!window.cloudProfile.friends.includes(fromUsername)) window.cloudProfile.friends.push(fromUsername);
    await window.saveCloudProfile();

    try {
      const q = query(collection(db, "users"), where("username", "==", fromUsername));
      const snap = await getDocs(q);
      if (!snap.empty) {
        const otherId = snap.docs[0].id;
        const otherData = snap.docs[0].data();
        const sent = (otherData.sentReqs || []).filter(u => u !== window.cloudProfile.username);
        const friends = otherData.friends || [];
        if (!friends.includes(window.cloudProfile.username)) friends.push(window.cloudProfile.username);
        await updateDoc(doc(db, "users", otherId), { sentReqs: sent, friends: friends });
      }
    } catch(e) { console.error(e); }
  ...
  async function removeFriend(username) {
    ...
    try {
      ...
      if (!snap.empty) {
        const otherId = snap.docs[0].id;
        const otherData = snap.docs[0].data();
        const friends = (otherData.friends || []).filter(u => u !== window.cloudProfile.username);
  ...
  function renderFriendCard(username, mode) {
    ...
    if (mode === "incoming") {
      ...
      if (status === "friends") actionHtml = `<button class="friend-action-btn friend-btn-friends" onclick="window.Friends.remove('${esc(username)}')" title="Remove friend">✓ Friends</button>`;
      else if (status === "pending") actionHtml = `<button class="friend-action-btn friend-btn-pending" onclick="window.Friends.cancel('${esc(username)}')">Pending ✕</button>`;
      else if (status === "incoming") actionHtml = `<div class="friend-request-btns"><button class="friend-btn-accept" onclick="window.Friends.accept('${esc(username)}')">✓</button><button class="friend-btn-decline" onclick="window.Friends.decline('${esc(username)}')">✕</button></div>`;
      else actionHtml = `<button class="friend-action-btn friend-btn-add" onclick="window.Friends.send('${esc(username)}')">+ Add</button>`;
    }

    return `
      <div class="friend-card">
        <div class="friend-avatar" style="background:${avatarColor(username)}">${avatarLetter(username)}<span class="friend-online-dot dot-offline"></span></div>
  ...
  function renderBadge() {
    ...
    if (reqBadge) { reqBadge.textContent = count; reqBadge.style.display = count ? "inline-flex" : "none"; }
  }
  ...
  function renderFriendsList() {
    ...
    body.innerHTML = `<div class="friends-section-label">Your Friends · ${friends.length}</div>` + friends.map((u) => renderFriendCard(u, "friend")).join('<div class="friends-divider"></div>');
  }
  ...
  function renderRequests() {
    ...
    if (incoming.length) {
      ...
      html += incoming.map((u) => renderFriendCard(u, "incoming")).join('<div class="friends-divider"></div>');
    }
    if (sent.length) {
      ...
      html += sent.map((u) => `
        <div class="friend-card">
          ...
          <div class="friend-info"><div class="friend-name">${esc(u)}</div><div class="friend-meta">📤 Awaiting response</div></div>
          <button class="friend-action-btn friend-btn-pending" onclick="window.Friends.cancel('${esc(u)}')">Pending ✕</button>
        </div>`).join('<div class="friends-divider"></div>');
    }
    if (!incoming.length && !sent.length) { html = `<div class="friends-empty"><div class="friends-empty-icon">📩</div>No pending requests.</div>`; }
    body.innerHTML = html;
  }
  ...
  async function renderFind(query) {
    ...
    body.innerHTML = `<div class="friends-section-label">People on FlixRate · ${users.length}</div>` + users.map((u) => renderFriendCard(u, "search")).join('<div class="friends-divider"></div>');
  }
  ...
  function switchTab(tabId) {
    activeTab = tabId;
    document.querySelectorAll(".friends-tab").forEach((t) => t.classList.toggle("active", t.dataset.ftab === tabId));
    const searchWrap = document.getElementById("friends-search-wrap");
    if (searchWrap) searchWrap.style.display = tabId === "find" ? "block" : "none";
    renderAll();
  }
  ...
  function init() {
    ...
    // Tabs
    document.querySelectorAll(".friends-tab").forEach((t) => t.addEventListener("click", () => switchTab(t.dataset.ftab)));
  ...
  return { init, open, close, send: sendRequest, cancel: cancelRequest, accept: acceptRequest, decline: declineRequest, remove: removeFriend };
})();
...
document.addEventListener('click', (e) => {
  ...
  if (e.target.closest('#friends-head-close') || e.target.id === 'friends-backdrop') {
