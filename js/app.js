...
const App = (() => {
    ...
    if (item.id && item.itemType) {
      ...
      card.addEventListener('mouseenter', () => {
        hoverTimer = setTimeout(async () => {
          ...
          try {
            let trailerKey = null;
            if (item.itemType === 'movie') trailerKey = await API.fetchMovieTrailer(item.id);
            else if (item.itemType === 'tv') trailerKey = await API.fetchTVTrailer(item.id);
  ...
  async function loadMovieRows() {
    ...
    if (movieRow) {
      ...
      } catch (e) { console.warn('Trending movies row error:', e.message); }
    }
    ...
    if (tvRow) {
      ...
      } catch (e) { console.warn('Trending TV row error:', e.message); }
    }
  }
  ...
  async function loadAnimeRows() {
    ...
    if (animeRow) {
      ...
      } catch (e) { console.warn('Top anime row error:', e.message); }
    }
    ...
    if (seasonRow) {
      ...
      } catch (e) { console.warn('Seasonal anime row error:', e.message); }
    }
  }
