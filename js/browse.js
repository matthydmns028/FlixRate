...
const Browse = (() => {
  ...
  const GENRES = {
    movie: [
      ...
      { id: 99,    name: 'Documentary', icon: '🎥', color: ['#065f46','#059669'] },
    ...
    tv: [
      { id: 10759, name: 'Action & Adventure', icon: '💥', color: ['#b91c1c','#dc2626'] },
      { id: 16,    name: 'Animation',  icon: '🎨', color: ['#7c3aed','#a855f7'] },
      { id: 35,    name: 'Comedy',     icon: '😄', color: ['#d97706','#fbbf24'] },
      { id: 80,    name: 'Crime',      icon: '🔫', color: ['#374151','#6b7280'] },
      { id: 99,    name: 'Documentary',icon: '🎥', color: ['#065f46','#059669'] },
      { id: 18,    name: 'Drama',      icon: '🎭', color: ['#1d4ed8','#3b82f6'] },
      { id: 10765, name: 'Fantasy & Sci-Fi', icon: '🔮', color: ['#5b21b6','#8b5cf6'] },
      ...
      { id: 10770, name: 'TV Movie',   icon: '🎬', color: ['#1f2937','#374151'] },
      { id: 37,    name: 'Western',    icon: '🤠', color: ['#92400e','#b45309'] },
    ],
    anime: [
      ...
      { id: 36,   name: 'Slice of Life',icon: '🌸', color: ['#be185d','#ec4899'] },
      { id: 30,   name: 'Sports',       icon: '⚽', color: ['#065f46','#059669'] },
      { id: 37,   name: 'Supernatural', icon: '👁️', color: ['#4c1d95','#7c3aed'] },
  ...
  function imgUrl(path) { return path ? `https://image.tmdb.org/t/p/w342${path}` : null; }
  ...
  async function fetchResults(append = false) {
    ...
    if (label) {
      label.textContent = currentGenre
        ...
        : (currentType === 'movie' ? '🎬 All Movies' : currentType === 'tv' ? '📺 All TV Shows' : '🎌 All Anime');
    }
    ...
    try {
      ...
      if (currentType === 'anime') {
        ...
        cards = await fetchTMDB(currentType, currentGenre?.id, currentSort, currentPage);
      }
      ...
      if (countEl && !append) countEl.textContent = `Page ${currentPage} / ${totalPages}`;
      ...
      isFetching = false;
      if (!append) grid.innerHTML = `<div class="browse-empty"><div class="browse-empty-icon">⚠️</div><div class="browse-empty-text">Failed to load results. Check your connection.</div></div>`;
    }
  }

  // ── TMDB fetch ────────────────────────────────────────────
  async function fetchTMDB(type, genreId, sort, page) {
    const sortMap = { popular: 'popularity.desc', rating: 'vote_average.desc', newest: 'primary_release_date.desc' };
    const sortTVMap = { popular: 'popularity.desc', rating: 'vote_average.desc', newest: 'first_air_date.desc' };
    const sortParam = (type === 'tv' ? sortTVMap : sortMap)[sort] || 'popularity.desc';
    ...
    const res = await fetch(endpoint, {
      headers: { Authorization: `Bearer ${CONFIG.TMDB_BEARER}`, accept: 'application/json' },
    });
  ...
  async function fetchAnime(genreId, sort, page) {
    ...
    if (sort === 'rating') url = url.replace('order_by=members', 'order_by=score');
    if (sort === 'newest') url = url.replace('order_by=members&sort=desc', 'order_by=start_date&sort=desc');
  ...
  function buildCardHTML(card) {
    const typeLabel = card.type === 'anime' ? '🎌 Anime' : card.type === 'tv' ? '📺 TV' : '🎬 Movie';
    const fallbackIcon = card.type === 'anime' ? '🎌' : card.type === 'tv' ? '📺' : '🎬';
    return `
      <div class="browse-card-poster-wrap">
        ${card.poster
          ? `<img src="${esc(card.poster)}" alt="${esc(card.title)}" class="browse-card-poster" loading="lazy">`
